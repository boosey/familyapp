...
/**
 * Behavior test for <HubTabsNav> — the hub header top tab bar (Task 3, Playful de-clutter).
 *
 ...
 * shared `?families=` browse filter (ADR-0021) when present and OMITTING it when absent. Overflow
 * entries (Invite / Requests) route the same way from the "More ▾" menu.
...
describe("HubTabsNav", () => {
  it("renders a Family tab", () => {
    render(<HubTabsNav primaryTabs={PRIMARY} overflowTabs={[]} active="stories" familiesParam={null} />);
    expect(screen.getByRole("tab", { name: "Family" })).toBeTruthy();
  });

  it("routes the Family tab preserving the ?families= filter when present", () => {
    render(
      <HubTabsNav primaryTabs={PRIMARY} overflowTabs={[]} active="stories" familiesParam="fam-marino" />,
    );
  ...
  it("routes an overflow entry from the More menu on the same in-page ?tab= route", () => {
    render(
      <HubTabsNav
        primaryTabs={PRIMARY}
        overflowTabs={[{ key: "requests", label: hub.shell.tabRequests }]}
        active="stories"
        familiesParam="fam-marino"
      />,
    );
