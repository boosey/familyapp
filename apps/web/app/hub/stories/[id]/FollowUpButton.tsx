...
import styles from "./FollowUpButton.module.css";
...
export function FollowUpButton({ storyId, targetPersonId, narratorName }: FollowUpButtonProps) {
  ...
  if (sent) {
    return (
      <p data-testid="follow-up-sent" className={styles.sent}>
        {hub.followUp.sent}
      </p>
    );
  }
  ...
  return (
    <form onSubmit={handleSubmit} data-testid="follow-up-form" className={styles.panel}>
      ...
      {error && (
        <p id="follow-up-error" role="alert" data-testid="follow-up-error" className={styles.errText}>
          {error}
        </p>
      )}
