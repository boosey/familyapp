...
### Design implications  
Every visible login screen on a narrator's side is a bug, not a feature — the magic link handles authentication invisibly. The interview engine must be built on oral-history and reminiscence technique — open-ended, concrete, non-leading, one question at a time — not generic chatbot patterns. Preserve and foreground her actual voice; synthesized prose is secondary to the recording. Build the consent and emotional-safety rails first. The narrator is the source of everything; protect that experience above all.
