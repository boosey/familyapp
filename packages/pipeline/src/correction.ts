/**
 * Voice correction (spec Part III): "a correction the narrator voices is applied to the prose (a
 ...
 * throughout; the narrator's NEXT voice action (approval) is what advances the story.
 */
import {
...
export interface ApplyVoiceCorrectionInput {
  ...
  storyId: string;
  /** The post-correction transcript (the prior transcript plus the narrator's spoken fix). */
...
export async function applyVoiceCorrection(
  ...
  const narrator = await getNarratorBiographicalContext(input.db, cleared.ownerPersonId);
  const render = await renderStoryFromTranscript(input.languageModel, {
    ...
    ...(narrator?.spokenName ? { narratorSpokenName: narrator.spokenName } : {}),
    ...(narrator && narrator.birthYear !== null
      ? { narratorBirthYear: narrator.birthYear }
      : {}),
  });

  // 3. Persist the regenerated derived fields. State stays `pending_approval`; the narrator must
