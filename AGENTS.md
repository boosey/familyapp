...
- `pnpm lint` (oxlint), `pnpm typecheck`, `pnpm test:all`, and `pnpm build` all pass on a clean tree. If lint/typecheck fail, check your own diff first — the baseline is green. (`pnpm test` at the workspace root is a no-op — root has no package tests; use `pnpm test:all` or `pnpm --filter <pkg> test`.)
