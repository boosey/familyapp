...
- `pnpm lint` (oxlint) and `pnpm typecheck` currently report some pre-existing errors (unused imports; a type-only import + test-file type mismatches). `pnpm test` (540 tests) and `pnpm build` pass. Don't assume you introduced lint/typecheck failures without checking `git stash`/diff first.
